...
def train_iter_hic(cfg, cell, chr, lstm_hidden_states, embed_rows):
    data_ob = DataPrepHic(cfg, cell, mode='train', chr=str(chr))
    ...
    for epoch_num in range(cfg.num_epochs):
        ...
        try:
            for i in range(start, stop):
                ...
                loss, hidden_state, seq_lstm_init, predicted_hic_subset, embed_row, r2 = unroll_loop(cfg,
                                                                                                 subset_hic_data,
                                                                                                 model.seq_lstm,
                                                                                                 seq_lstm_optimizer,
                                                                                                 criterion,
                ...
                if loss != 0:
                    monitor.monitor_loss_iter(callback, loss, r2, iter_num)
...
def unroll_loop(cfg, subset_hic_data, seq_lstm, seq_lstm_optimizer,
    ...
    r2 = 1 - (ss_res / ss_tot)
    return mean_loss, hidden_state_row, seq_lstm_init, predicted_hic_subset, embed_row, r2


if __name__ == '__main__':
    setup_logging()

    train_chr = list(range(21, 23))
    #train_chr.remove(11)
    # train_chr = [20, 22]
    ...
    for chr in train_chr:
        logger.info('Training Start Chromosome: {}'.format(chr))
        lstm_hidden_states, embed_rows, predicted_hic_data = train_iter_hic(cfg, cell, chr, lstm_hidden_states, embed_rows)

        #np.save(cfg.hic_path + "GM12878/" +str(chr) + "/" + "predicted_exp_hic_" + str(chr) + ".npy", predicted_hic_data)
